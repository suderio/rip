...
public class RipServerTest {
  ...
  public static void setUpBeforeClass() throws Exception {
    ...
    localhost().post("/test").contains("teste1")
        .respond(withFile("/teste1.json"));
    localhost().post("/test").contains("teste2").and().contains("something")
        .respond(withFile("/teste2.json"));
    ...
    localhost().delete("/test").contains("xpto").or().contains("abcd")
        .respond("Ok", SC_OK);
  ...
  public void testDelete() {
    given().body("something abcd something...").when().delete("/test").then()
        .statusCode(SC_OK);
    given().body("something xpto something...").when().delete("/test").then()
        .statusCode(SC_OK);
