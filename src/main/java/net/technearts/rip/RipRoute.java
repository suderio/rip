 ...
 * 
 * Um RipRoute é responsável por decidir qual resposta enviar a qual requisição. Isso é feito
 * através de um <code>Predicate&lt;Request&gt;</code> que decide qual <code>RipResponse</code> será
 * usado para gerar a resposta.
 * 
 * 
 */
public class RipRoute implements Comparable<RipRoute>, AutoCloseable {
  ...
  private static final Map<RipRoute, Map<Predicate<Request>, RipResponse>> CONDITIONS =
      new LinkedHashMap<>();
  private static final Map<RipRoute, BiFunction<Request, Response, String>> LOGS =
      new LinkedHashMap<>();
  ...
  /**
   * Configura a condição para que essa rota seja usada, e a resposta que será enviada.
   * 
   ...
   * 
   * @param log Uma função que constrói a mensagem de log a partir do request e response
   */
  ...
  /**
   * Constói um <code>Route</code> que verifica dinamicamente se deve ser chamado em determinada
   * rota.
   * 
   * @return Um <code>spark.Route</code> que será usado no servidor para responder à rota
   */
  private Route getRoute() {
    return (req, res) -> {
      final Optional<Map.Entry<Predicate<Request>, RipResponse>> optional = CONDITIONS.get(this)
          .entrySet().stream().filter(entry -> entry.getKey().test(req)).findFirst();
  ...
  /**
   * Constói um <code>TemplateViewRoute</code> que verifica dinamicamente se deve ser chamado em
   * determinada rota.
   * 
   * @return Um <code>spark.TemplateViewRoute</code> que será usado no servidor para responder à
   *         rota
   */
  private TemplateViewRoute getTemplateRoute() {
    return (req, res) -> {
      ...
      if (optional.isPresent()) {
        ...
        for (final Map.Entry<String, Function<Request, String>> f : response.getAttributes()
            .entrySet()) {
