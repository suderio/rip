...
public class RipResponseBuilder {
  ...
  RipResponseBuilder(final RipRoute route) {
    logger.info("Criando RipResponseBuilder para requisição {} {}",
    ...
    if (!routes.containsKey(route)) {
      routes.put(route, (req, res) -> {
        ...
        if (optional.isPresent()) {
          ...
          logger.debug("Resposta para {} {} não encontrada", route.getMethod(),
              route.getPath());
          result = "";
        }
  ...
  private void createTemplateMethod() {
    ...
    switch (route.getMethod()) {
    case connect:
      route.getRipServer().service.connect(route.getPath(),
          templateRoutes.get(route), templateEngine);
      break;
    case delete:
      route.getRipServer().service.delete(route.getPath(),
          templateRoutes.get(route), templateEngine);
      break;
    case get:
      route.getRipServer().service.get(route.getPath(),
          templateRoutes.get(route), templateEngine);
      break;
    case head:
      route.getRipServer().service.head(route.getPath(),
          templateRoutes.get(route), templateEngine);
      break;
    case options:
      route.getRipServer().service.options(route.getPath(),
          templateRoutes.get(route), templateEngine);
      break;
    case patch:
      route.getRipServer().service.patch(route.getPath(),
          templateRoutes.get(route), templateEngine);
      break;
    case post:
      route.getRipServer().service.post(route.getPath(),
          templateRoutes.get(route), templateEngine);
      break;
    case put:
      route.getRipServer().service.put(route.getPath(),
          templateRoutes.get(route), templateEngine);
      break;
    case trace:
      route.getRipServer().service.trace(route.getPath(),
          templateRoutes.get(route), templateEngine);
      break;
    case after:
    case afterafter:
